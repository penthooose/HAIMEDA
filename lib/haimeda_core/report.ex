defmodule HaimedaCore.Report do
  ...
  def update_report_section(report_id, section, section_id, data) do
    ...
    try do
      ...
      # IO.inspect(update_data, label: "Update Data before save")

      case section do
        "general" ->
          # For general section, use the simple field update
          Mongo.update_one(
            Repo.get_conn(),
            @reports_collection,
            %{_id: mongo_id},
        ...
        "chapters" ->
          # For chapters, check if the item exists
          if section_item_exists?(report_id, section, section_id) do
            # Update each field individually to avoid overwriting other fields
            update_fields =
              update_data
            ...
            # IO.inspect(update_fields, label: "Update Fields for chapters")
            ...
            # Insert new chapter if it doesn't exist
            Mongo.update_one(
              Repo.get_conn(),
              @reports_collection,
              %{_id: mongo_id},
        ...
        "parties" ->
          # For parties, check if item exists then update
          if section_item_exists?(report_id, section, section_id) do
            Logger.debug("Party exists - updating existing party")

            # Build update operation with all fields to ensure complete replacement
            update_operation = %{
              "$set" =>
                update_data
                |> Enum.map(fn {key, value} ->
  ...
  This function replaces the non-existent set_report/2 function
  """
  def save_report(report_id, report_data) do
    require Logger
    Logger.info("Saving report #{report_id}")

    # Assuming the reports are stored in a local database
    # If using a different storage mechanism, adapt this code accordingly
    try do
      # Build file path for the report
  ...
  defp process_doc_for_transfer(doc) do
    doc
    # Remove any struct metadata
    |> Map.delete(:__struct__)
    |> Enum.reduce(%{}, fn
      # Each pattern must be a separate clause in the function
      {k, %DateTime{} = dt}, acc ->
        Map.put(acc, k, dt)
      ...
      # Process lists by traversing them

defmodule RAM.PromptBuilder do
  ...
  alias RAM.PromptBuilder, as: PromptBuilder
  ...
  def create_prompt(prompt_key, input_info, type \\ nil) do
    case prompt_key do
      "chapter_creation" ->
        PromptBuilder.create_chapter_prompt(prompt_key, input_info, type)

      "text_optimization" ->
        PromptBuilder.create_text_optimization_prompt(prompt_key, input_info)

      "text_revision" ->
        PromptBuilder.create_text_revision_prompt(prompt_key, input_info)

      "text_summarization" ->
        PromptBuilder.create_text_summarization_prompt(prompt_key, input_info)

      "user_request" ->
        PromptBuilder.create_user_request_prompt(prompt_key, input_info, type)
  ...
  def create_user_request_prompt(prompt_key, input_info, type) do
    case type do
      :no_context ->
        PromptBuilder.create_user_question_prompt_no_context(prompt_key, input_info)

      :with_context ->
        PromptBuilder.create_user_question_prompt_with_context(prompt_key, input_info)
  ...
  def create_user_question_prompt_with_context(prompt_key, input_info) do
    ...
    # IO.inspect(input_info.context, label: "Context in PromptBuilder")
    # IO.inspect(context_snippet_template, label: "Context Snippet Template")
    # IO.inspect(context_string, label: "Context String")
  ...
  def create_text_revision_prompt(prompt_key, input_info) do
    ...
    IO.inspect(formatted_missing_entities, label: "Formatted Missing Entities")
  ...
  def create_chapter_prompt(prompt_key, input_info, previous_contents_type) do
    case previous_contents_type do
      :summaries ->
        PromptBuilder.create_chapter_prompt_with_summaries(prompt_key, input_info)

      :full_chapters ->
        PromptBuilder.create_chapter_prompt_with_full_chapters(prompt_key, input_info)

      nil ->
        PromptBuilder.create_chapter_prompt_without_previous_content(prompt_key, input_info)
  ...
  def create_chapter_prompt_with_summaries(prompt_key, input_info) do
    ...
    IO.inspect(parties_statements, label: "Parties Statements")
    ...
    IO.inspect(previous_content, label: "Previous Content in building")

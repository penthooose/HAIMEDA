    ...
    |> Enum.reduce([], fn output_entity, acc ->
      ...
      cond do
        ...
        true ->
          [
            %{
              # Fixed: Use output_entity.entity instead of output_entity.output_entity
              output_entity: output_entity.entity,
    ...
    {marked_text, markers} =
      Enum.reduce(sorted_entities, {text, %{}}, fn ent, {current_text, markers} ->
        ...
        if replaced_text != current_text do
          # FIXED: Preserve the original replacements instead of filtering aggressively
          # Only filter out replacements with § to avoid JSON corruption

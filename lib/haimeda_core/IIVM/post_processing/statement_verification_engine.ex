defmodule PostProcessing.StatementVerificationEngine do
  ...
  # Instead of using a module attribute, create a function that returns the rules
  def override_rules do
    [
      # If perfect keyword overlap with high similarity keyword, upgrade to strong match
  ...
  # Extracted function to safely check for high similarity keywords
  defp keyword_has_high_similarity(result) do
    if result.overlap_percent == 100 && is_list(result.keywords.common) do
      Enum.any?(result.keywords.common, fn keyword ->
        ...
        # Now safely apply the regex to the string
  ...
  # Process the batch results from Python into Elixir-friendly format
  defp process_batch_results(results) do
    # First convert any charlists to strings
    string_results = convert_charlists(results)

    # IO.inspect(string_results, label: "String Results")

    # Map each result to the expected format for comparison
    Enum.map(string_results, fn result ->
      # Ensure statement fields are strings
      statement1 =
      ...
      # Extract metrics from result structure with proper fallbacks
      metrics = %{
      ...
      # Extract keywords with proper fallbacks and ensure they're properly formatted
  ...
  # New helper function to get and format keywords consistently
  ...
  # Helper function to get a string value from a map, handling charlists
  defp get_string_value(map, key, default) do
    case Map.get(map, key) do
      ...
      value when is_list(value) ->
        # Move the List.ascii_printable? check outside the guard clause
  ...
  # Helper function to ensure proper string conversion
  defp convert_to_string(value) when is_list(value) do
    # Move the List.ascii_printable? check outside the guard clause
    if List.ascii_printable?(value) do
      List.to_string(value)
    else
      # Try harder to convert lists that might represent special characters
  ...
  # New helper function to try harder to convert charlists with special characters
  defp try_convert_charlist(value) when is_list(value) do
    # Try to interpret as UTF8 codepoints
    try do
      value
  ...
  defp try_convert_charlist(value), do: to_string(value)

  # Helper function to convert charlists to strings recursively
  defp convert_charlists(item) do
    cond do
      ...
      is_list(item) and Enum.all?(item, fn x -> is_integer(x) and x >= 0 and x <= 0x10FFFF end) ->
        # Try harder for lists that might be UTF-8 code points
  ...
  # New helper function to properly format keywords from Python
  ...
  # New helper function to build a lookup map of comparison results
  defp build_comparison_results_lookup(batch_results, input_statements, output_statements) do
    # Create maps to lookup input/output statements by their entity content
    input_map =
    ...
    # Build lookup table with {input_id, output_id} as the key
    Enum.reduce(batch_results, %{}, fn result, acc ->
      # Extract safely - handle both map format and struct format
      ...
      if input_id && output_id do
        # Extract result or use the entire item if it's already the result

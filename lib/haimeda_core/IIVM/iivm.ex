defmodule IIV do
  ...
  def pre_process_input_to_LLM(input_info) do
    ...
    IO.inspect(filename_class, label: "Filename Class")
      ...
      case File.read(@filename_categories) do
        {:ok, categories_json} ->
          case Jason.decode(categories_json) do
            {:ok, categories} ->
              # Look for the first match of the filename in any category
              Enum.find_value(categories, nil, fn {category, filenames} ->
                # Return the category name if the sanitized filename matches

defmodule PreProcessing.TableauxSolver do
  ...
  require Tableaux
  # Import everything except subset/2
  ...
  # Special case for extract_cond - returns just the list of IDs
  ...
  def solve(formula, debug) when is_boolean(debug) do
    # Handle old boolean debug parameter for backwards compatibility
  ...
  # Add comparison helper function
  defp compare_values(op, x, y) do
    # Handle boolean comparisons first
    if is_boolean(x) or is_boolean(y) do
      ...
      # Fall back to numeric comparison for non-boolean values
      case op do
        :eq -> x == y
        :neq -> x != y
        :lt -> x < y
  ...
  # Add range value comparison helper
  defp compare_values(:range, value, [min_val, max_val]) do
    value >= min_val and value <= max_val
  end

  # Update compare_values to focus on numeric comparisons
  ...
  # Add helper for value normalization

defmodule RIM.QueryProcessor do
  alias RIM.{RequestHandler, VectorPersistence, ResponseGenerator}
  alias RIM.SymbolicWordProcessor, as: SWP

  def process_query(query_processor_map) do
    # Extract the relevant information from the query_processor_map
    ...
    # Update the query_processor_map with the queries and specific_report_ids
    query_processor_map =
      query_processor_map
  ...
  def process_report_query(query_processor_map) do
    ...
    {report_ids, mdb_records} =
      case specific_report_ids do
        [] ->
          ...
          # Extract only the IDs from the tuple returned by determine_possible_report_ids
    ...
    # RequestHandler.extract_report_data_with_id(ids)
    IO.inspect(report_ids, label: "Report IDs")
    # IO.inspect(mdb_records, label: "MDB Records")
      ...
      case length(report_ids) do
        n when n > 0 and n < 3 ->
          ...
          if combine_mdb_and_vector_results do
            # If combine flag is true, add sizes together
            ...
            mdb_records =
              case combined_results_size do
                ...
                _ ->
                  # Return original records when size is acceptable
                  mdb_records
              end
            ...
            # If combine flag is false, choose either vector or MDB results
            # Prefer vector results for RAG when available
            if vector_results_size > 0 and vector_results_size <= max_size do
              # Use only vector results
              {nil, vector_results, :vector_results}
            else
              # Fallback to MDB results if vector results are too large or empty
              if mdb_records_size <= max_size do
                {mdb_records, nil, :mdb_results}
              else
                # Try to reduce MDB records if they're too large
                reduced_mdb_records =
                  decrease_record_sizes(
                    mdb_records,
                    hp_table_columns,
  ...
  def query_mdb_table_records_with_priorities(queries) do
    ...
    # For now, just initialize the variables and return empty list
    ...
    combined_search_results =
      RequestHandler.combine_search_results(
        hp_search_results,
        lp_search_results
      )

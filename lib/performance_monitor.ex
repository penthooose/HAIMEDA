defmodule HaimedaCore.PerformanceMonitor do
  ...
  def start_link(opts \\ []) do
    # Attempt to start OS_Mon application for CPU monitoring
    :application.ensure_started(:os_mon)

    # Check if GPU monitoring is available on startup
  ...
  defp get_gpu_usage do
    try do
      ...
      # Parse the output (expecting format like "30, 4000, 8000")
      case String.split(String.trim(util_output), ",") do
        [utilization, memory_used, memory_total] ->
          # Safely parse each value with better error handling
          ...
          # Only return the structure if all values parsed successfully

defmodule HaimedaCoreWeb.ReportsEditor.TabManagement do
  alias HaimedaCoreWeb.ReportsEditor.{ContentPersistence, Editor, PreviousContent}
  ...
  def handle_event("save-content", %{"id" => tab_id, "value" => value}, socket) do
    ...
    if updated_tab do
      ...
      # Return data for Editor to assign
      {:tabs, tabs}
    else
      {:error, "Tab not found"}
    end
  ...
  def handle_select_section_item(socket, item_id, category) do
    ...
    if existing_tab do
      ...
      # Make sure the tab has active_meta_info field

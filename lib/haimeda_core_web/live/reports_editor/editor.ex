defmodule HaimedaCoreWeb.ReportsEditor.Editor do
  ...
  # Helper function to safely get type information
  defp typeof(term) do
    cond do
  ...
  # Add a private helper function to initialize button states for correction mode
  def correction_mode_initiate(socket) do
    # Set appropriate button states for correction mode
  ...
  # Add a new handler for direct version navigation
  def handle_info({:navigate_to_version, version_num}, socket) do
    ...
    if tab do
      ...
      # Reload the tab to get the latest data
      ...
      # Force refresh the editor to show the new content
      socket =
        socket
  ...
  def handle_event("ai-correction", %{"id" => tab_id}, socket) do
    ...
    if tab do
      # Disable buttons during the operation - now false means disable
      disable_all_button_states()

      # Set loading state with specific message for AI correction
      socket = assign(socket, :loading, true)
      socket = assign(socket, :loading_message, :ai_correction)

      # Extract missing entities from formatted_content instead of using the plain textarea_content
      formatted_content = Map.get(tab, :formatted_content, %{})
      IO.inspect(formatted_content, label: "Formatted Content")

      # Extract entities that are confirmed=true and deleted=false
      ...
      # Use the clean text instead of the original content
  ...
  def handle_info({:chapter_summarization_complete, tab_id}, socket) do
    ...
    # No need to trigger additional events - complete the process here
    {:noreply, socket}
  end

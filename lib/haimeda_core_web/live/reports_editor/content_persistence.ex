defmodule HaimedaCoreWeb.ReportsEditor.ContentPersistence do
  alias HaimedaCoreWeb.ReportsEditor.{Editor, TabManagement, PreviousContent}
  alias Jason
    ...
    case tab do
      ...
      %{category: "chapters", section_id: section_id, content: content} ->
        ...
        updated_versions =
          Enum.map(chapter_versions, fn version ->
            if Map.get(version, "version") == current_version_index do
              ...
              formatted_content =
                if Map.has_key?(tab, :formatted_content) && tab.formatted_content != nil do
                  # Log what we're actually saving to the database
                  fc_string =
        ...
        if current_version do
          # Fix: safely access nested structure using pattern matching instead of get_in with list indices
        ...
        # Build update data - DON'T include a formatted_content field at the root level
        # to avoid duplicate content. The formatted_content is already in the chapter_versions array
        update_data = %{
          "id" => section_id,
          "title" => tab.label,
        ...
        # Log the result of the MongoDB update
  ...
  def create_default_formatted_content(plain_text) do
    ...
    if plain_text == "" do
      ...
      # Instead of splitting by paragraphs, process the entire text as a single unit with hardBreaks
      ...
      content_nodes =
        ...
        |> Enum.flat_map(fn {segment, idx} ->
          ...
          if is_last do
            ...
            # Use proper hardBreak node AND text node with space
            [
              text_node,
              %{"type" => "hardBreak"}
              # Explicit space after hardBreak
              # %{"type" => "text", "text" => " "}
            ]
          end
        end)
  ...
  def verify_formatted_content(formatted_content) do
    case formatted_content do
            ...
            %{"type" => "paragraph", "content" => para_content} = para when is_list(para_content) ->
              ...
              fixed_content =
                ...
                |> Enum.filter(fn
                  ...
                  %{"type" => "text"} -> true
                  # CHANGED: Keep hardBreak nodes instead of filtering them out
                  %{"type" => "hardBreak"} -> true
      ...
      _ ->
        # If the formatted content is completely broken, create a default document
  ...
  @doc """
  Updates content with rich formatting. Unlike create_default_formatted_content,
  this preserves complex structures like entity marks and formatting.
  """
  ...
  # Update the save_chapter_version function to create empty new versions
  ...
  # Update the delete_chapter_version function to handle version deletion properly
  def delete_chapter_version(report_id, section_id, version_to_delete) do
    ...
    # Handle special case: if we deleted the last version, create a new empty one
    updated_versions =
  ...
  # Helper to get existing chapter data - FIX THE DEFAULT CURRENT_VERSION TO BE 1 INSTEAD OF 0
    ...
    case Report.get_report(report_id) do
      {:ok, report} ->
        ...
        if chapter do
          ...
          # Default structure for a new chapter - IMPORTANT: current_version should be 1, not 0!
          %{
            "id" => section_id,
            "title" => "",
            "chapter_info" => "",
      ...
      {:error, _} ->
        # Default structure for error case - IMPORTANT: current_version should be 1, not 0!
        %{
          "id" => section_id,
          "title" => "",
          "chapter_info" => "",
    ...
    content_nodes =
      ...
      |> Enum.flat_map(fn {segment, idx} ->
        ...
        if is_last do
          ...
          # Use proper hardBreak node AND space after it
          [
            text_node,
            %{"type" => "hardBreak"}
            # %{"type" => "text", "text" => " "}

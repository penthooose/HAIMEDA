defmodule HaimedaCoreWeb.ReportsEditor.OutputArea do
  ...
  alias HaimedaCoreWeb.ReportsEditor.{ContentPersistence, TiptapEditor, TiptapSnippets}
  require Logger
  ...
  def update(%{ai_correction_disabled: state} = assigns, socket) do
    ...
    send(socket.parent_pid, {:update_button_state, :ai_correction_disabled, state})
  ...
  def update(%{confirm_changes_disabled: state} = assigns, socket) do
    ...
    send(socket.parent_pid, {:update_button_state, :confirm_changes_disabled, state})
  ...
  def update(%{discard_changes_disabled: state} = assigns, socket) do
    ...
    send(socket.parent_pid, {:update_button_state, :discard_changes_disabled, state})
  ...
  def update(%{auto_chapter_disabled: state} = assigns, socket) do
    ...
    send(socket.parent_pid, {:update_button_state, :auto_chapter_disabled, state})
  ...
  def update(%{manual_verification_disabled: state} = assigns, socket) do
    ...
    send(socket.parent_pid, {:update_button_state, :manual_verification_disabled, state})
  ...
  def update(%{ai_optimize_disabled: state} = assigns, socket) do
    ...
    send(socket.parent_pid, {:update_button_state, :ai_optimize_disabled, state})
  ...
  def previous_version(socket) do
    send(socket.parent_pid, {:navigate_version, :previous})
    socket
  end

  def next_version(socket) do
    send(socket.parent_pid, {:navigate_version, :next})
    socket
  end

  def update_textarea(socket, content, mode \\ "writable") do
    send(socket.parent_pid, {:update_textarea_field, content, mode})
    socket
  end

  def set_textarea_mode(socket, mode) do
    send(socket.parent_pid, {:set_textarea_mode, mode})
    socket
  end

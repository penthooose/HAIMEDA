defmodule HaimedaCoreWeb.ReportsLive.Index do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      # Only fetch reports when the client is connected
  ...
  def handle_event("create-report", %{"name" => name}, socket) do
    ...
    case Report.create_report(%{name: report_name}) do
      {:ok, new_report} ->
        ...
        # Add a delay before fetching reports to ensure the new report is visible
        Process.sleep(100)

        # Reload all reports to ensure correct sorting
        reports = fetch_reports()
  ...
  def handle_event("open-report", %{"id" => id}, socket) do
    # Reset LLM initialization status before opening the report
    # This ensures the LLM integration will be performed again if needed
